 ...
 * account, and for a site with no inbound links yet it is the fastest route to
 * being crawled by them at all. Google does not take part — for Google the
 * levers are the sitemap in robots.txt, Search Console, and links.
 *
...
const HOST = process.env.INDEXNOW_HOST || 'mailmint.app.mintapis.com';

// Read the URLs out of our own sitemap rather than a second list here, so the
...
async function main() {
  ...
  // The key file has to be reachable and hold exactly the key, or every
  // submission is rejected as unverified — check before sending anything.
